...
- Inc10 前提の session reuse 安全ガードと reuse 機構本体 (IncE, §14.5/§16.4/§26.3)
...
## Session Reuse (IncE, Inc10 前提)

### ClaudeSessionReuseEligibleQ[sessionId, newStartSpec] → Association
open session を新 episode に再利用してよいか判定する (Inc10 前提の安全ガード)。全条件 AND で eligible: ReusePolicy==SameWorkflowSameTrust / 同一 WorkflowId / 同一 PolicySnapshotHash / 新 backend の InferenceTrustDomain が session 累積 PrivacyLabel を受容 (§16.4 taint 単調) / Prepared な非冪等 tool effect が残っていない (I9)。
→ `<|"Eligible"->_, "Reasons"->{...}, "CarryForward"->_|>`
reuse 本体 (ReusePolicy=SameWorkflowSameTrust の実施) は v0.1 非目標で本関数は判定のみ。

### ClaudeSessionRaiseAccumulatedPrivacy[sessionId, label] → PrivacyLabel
session の累積 PrivacyLabel を単調に引き上げる (§16.4)。読んだ input/artifact/tool result の最大値へ。下げられない。現在値を返す。

### ClaudeRuntimeReuseSessionForEpisode[sessionId, newStartSpec] → Association
Inc10 本体 = session reuse の機構。終端 (Completed) の生きた session を、同一 workflow / 同一 trust domain の次 episode 用に再オープンする。ClaudeSessionReuseEligibleQ の gate を通り、eligible なら per-episode 状態 (RuntimeId/Journal/NextSeq/Result 等) を reset しつつ、累積の BudgetUsed.ToolCalls と AccumulatedPrivacyLabel は reset せず carry forward する (Inc10 受け入れ)。物理 session id は維持し、Status を "Open" に戻して次の StartEpisode を可能にする。『いつ reuse するか』の判断は Petri net の ReusePolicy (Orchestrator) の領分で、本関数は worker runtime 側の機構のみを提供する。
→ `<|"Status"->("Reused"|"Ineligible"|"NotReusableYet"|"NoSession"|"Disposed"), "SessionId"->_, "EpisodeId"->_, "CarriedToolCalls"->_, "AccumulatedPrivacyLabel"->_, "EpisodeCount"->_, "Reasons"->{...}|>`
